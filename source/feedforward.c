...
/* Allocate (or re-allocate) the memory for the input layer. */
	...
	pt_feedforward->pt_input_layer_buffer = realloc(pt_feedforward->pt_input_layer_buffer, sizeof(float_t) * t_input_layer_size);
 ...
 */
	...
	pt_feedforward->ppt_hidden_layer_weight_buffer = realloc(pt_feedforward->ppt_hidden_layer_weight_buffer, sizeof(float_t *) * pt_feedforward->t_hidden_layer_weight_buffer_size);
...
/* Allocate the memory for the weight matrix. */
		pt_feedforward->ppt_hidden_layer_weight_buffer[l_i] = realloc(pt_feedforward->ppt_hidden_layer_weight_buffer[l_i], sizeof(float_t) * l_last_layer_size * l_next_layer_size);
	}
...
/* Allocate the pointers for the bias vectors for each of the hidden layers. */
	...
	pt_feedforward->ppt_hidden_layer_bias_buffer = realloc(pt_feedforward->ppt_hidden_layer_bias_buffer, sizeof(float_t *) * pt_feedforward->t_hidden_layer_weight_bias_size);
...
/* Allocate the memory for the bias vectors for each of the hidden layers. */
	for(uintmax_t l_i = 0; l_i < pt_feedforward->t_hidden_layer_weight_bias_size; l_i++) pt_feedforward->ppt_hidden_layer_bias_buffer[l_i] = realloc(pt_feedforward->ppt_hidden_layer_bias_buffer[l_i], sizeof(float_t) * pt_feedforward->pt_hidden_layer_size_buffer[l_i]);

/* Allocate the memory for the output layer. */
	pt_feedforward->t_output_layer_buffer_size = t_output_layer_size;
	pt_feedforward->pt_output_layer_buffer = realloc(pt_feedforward->pt_output_layer_buffer, sizeof(float_t) * t_output_layer_size);
...
/* Allocate the memory for the weight matrix of the output layer. */
	...
	pt_feedforward->pt_output_layer_weight_buffer = realloc(pt_feedforward->pt_output_layer_weight_buffer, sizeof(float_t) * t_output_layer_size * pt_feedforward->pt_hidden_layer_size_buffer[t_hidden_layer_size_buffer_size - 1]);
...
/* Allocate the memory for the bias vector of the output layer. */
	...
	pt_feedforward->pt_output_layer_bias_buffer = realloc(pt_feedforward->pt_output_layer_bias_buffer, sizeof(float_t) * t_output_layer_size);
...
/* The Xavier scaling factor. */
		float_t l_scale = sqrt(2.0 / (l_last_layer_size + l_next_layer_size));

/* Initialize the weights. */
		for(uintmax_t l_j = 0; l_j < l_last_layer_size * l_next_layer_size; l_j++)
		{
			float_t l_random = (float_t)rand() / (float_t)RAND_MAX * 2.0 - 1.0;
...
/* Initialize the output layer weights. */
	...
	float_t l_output_scale = sqrt(2.0 / (l_last_hidden_size + pt_feedforward->t_output_layer_buffer_size));
...
/* Copy the input buffer to the input layer. */
	memcpy(pt_feedforward->pt_input_layer_buffer, pt_input_buffer, sizeof(float_t) * t_input_buffer_size);

/*
 *	Keep track of the activation buffer for our current layer & the next layer.
 */
	float_t *pl_current_layer_activation_buffer = pt_feedforward->pt_input_layer_buffer;
	uintmax_t l_current_layer_activation_buffer_size = pt_feedforward->t_input_layer_buffer_size;
	float_t *pl_next_layer_activation_buffer = NULL;
 ...
 */
		...
		pl_next_layer_activation_buffer = malloc(sizeof(float_t) * l_next_layer_activation_buffer_size);
...
/* Copy the output layer buffer to the attached output buffer. */
	memcpy(pt_output_buffer, pt_feedforward->pt_output_layer_buffer, sizeof(float_t) * t_output_buffer_size);
 ...
 */
	float_t **ppl_hidden_activation_buffer_buffer = malloc(sizeof(float_t *) * ppl_hidden_activation_buffer_buffer_size);
...
/* The buffer that keeps track of the previous layer's activation buffer. */
	float_t *pl_previous_layer_activation_buffer = pt_feedforward->pt_input_layer_buffer;
...
/* Allocate memory for the current hidden layer's activation buffer. */
		...
		float_t *pl_current_layer_activation_buffer = malloc(l_current_layer_activation_buffer_size * sizeof(float_t));
 ...
 *	it's inputs.
 */
	float_t *pl_delta_output_buffer = malloc(pt_feedforward->t_output_layer_buffer_size * sizeof(float_t));
...
/* Compute the error gradients for each output. */
	...
	{
/* Calculate the error. */
		float_t l_error = pt_feedforward->pt_output_layer_buffer[l_j] - pt_target_buffer[l_j];
 ...
 */
	float_t *pl_last_hidden_activation_buffer = (ppl_hidden_activation_buffer_buffer_size > 0) ? ppl_hidden_activation_buffer_buffer[ppl_hidden_activation_buffer_buffer_size - 1] : pt_feedforward->pt_input_layer_buffer;
 ...
 */
	float_t *pl_delta_next_layer_buffer = pl_delta_output_buffer;
 ...
 */
		...
		float_t *pl_delta_current_layer_buffer = malloc(l_delta_current_layer_buffer_size * sizeof(float_t));
 ...
 */
			if(l_i == (intmax_t)(ppl_hidden_activation_buffer_buffer_size - 1))
			{
				...
				{
					float_t l_weight = pt_feedforward->pt_output_layer_weight_buffer[l_j * l_delta_next_layer_buffer_size + l_k];
			...
			else
			{
				...
				{
					float_t l_weight = pt_feedforward->ppt_hidden_layer_weight_buffer[l_i + 1][l_j * l_next_hidden_layer_size + l_k];
					l_sum += l_weight * pl_delta_next_layer_buffer[l_k];
				}
...
/* Retrieve the activation value for the current node. */
			float_t l_activation = ppl_hidden_activation_buffer_buffer[l_i][l_j];
/*
...
/* ... */
		float_t *pl_previous_activation_layer_buffer = (l_i == 0) ? pt_feedforward->pt_input_layer_buffer : ppl_hidden_activation_buffer_buffer[l_i - 1];
